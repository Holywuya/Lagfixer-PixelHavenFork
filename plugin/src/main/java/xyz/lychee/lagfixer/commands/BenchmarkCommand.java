...
public class BenchmarkCommand extends CommandManager.Subcommand {
    ...
    public BenchmarkCommand(CommandManager commandManager) {
        super(commandManager, "benchmark", "运行基准测试并与其他服务器比较", "test");
    }
    ...
    public boolean execute(@NotNull org.bukkit.command.CommandSender sender, @NotNull String[] args) {
        ...
        if (monitor.getMspt() > 10.0) {
            return MessageUtils.sendMessage(true, sender, "&7服务器 MSPT 过&c高&7，结果可能不准确！");
        }

        long availableRam = monitor.getRamFree() + (monitor.getRamMax() - monitor.getRamTotal());
        if (availableRam < 2048) {
            return MessageUtils.sendMessage(true, sender, "&7服务器可用内存不足，需要 &c" + availableRam + "&8/&c2048MB");
        }
        ...
        Thread thread = new Thread(() -> {
            try {
                ...
                MessageUtils.sendMessage(true, sender, "&7基准测试完成，耗时 &f" + t.stop().getExecutingTime() + "&7ms，结果：&f" + result);
    ...
    public Benchmark runBenchmarks(int warmup, int cpu, int arrayLength, int memoryPasses) {
        ...
        benchmark.getResult()
                ...
                .append("\n &8• &f最佳时间：&e").append(bestScore / 1_000_000_000D).append(" s")
                .append("\n &8• &f最差时间：&e").append(worstScore / 1_000_000_000D).append(" s");
        ...
        // RAM Benchmark
        benchmark.getResult().append("\n \n&8&m    &r&8[ &eLagFixer 高级 RAM 基准测试 &8]&m    &r\n ");
